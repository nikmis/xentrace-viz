void drawLineChartArea(float v_hue, float[] temp, float[] data, int nth_histogram)
{
  hover_flag = 0;
  int i;
  ...
  for (i = (int)(begin_f+slide);  (i < (int)(end_f+slide)) && (i < data_size - 1); i++, j+=bar_width)
  {
    ...
    // x - axis
    line(graphs_start_x+2, nth_histogram*(height - slider_height - r_slider_y)/num_histograms, 
    width, nth_histogram*(height - slider_height - r_slider_y)/num_histograms);
    ...
    vertex(j + bar_width/2, nth_histogram*(height - slider_height - r_slider_y)/num_histograms - temp[i]);
  ...
  stroke(v_hue, 255, 200);
  fill(v_hue, 255, 200, 35);

  vertex(width, nth_histogram*(height - slider_height - r_slider_y)/num_histograms);
  vertex(graphs_start_x, nth_histogram*(height - slider_height - r_slider_y)/num_histograms);
